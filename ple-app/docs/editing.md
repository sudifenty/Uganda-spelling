...
> **⚠️ Updated 27 Aug 2026.** The section below ("Which folder is which") now matches the repo
> as it actually is. Line numbers further down in this guide refer to an older, much smaller
> version of `index.html` (~1,388 lines). The file is now ~4,000 lines, so use **Ctrl+F search,
> not line numbers**. The principles (colour slots, the edit loop) still hold.

Everything lives in **one file**: `ple-app/index.html` (~4,000 lines, all data embedded).
The reliable way to change it is:

```
edit the SOURCE  (ple-app/notes/*.md, ple-app/data/*.json, ple-app/tools/*.js, or
                  ple-app/index.html OUTSIDE the module markers)
        ↓
cd ple-app && python3 tools/build_all.py     ← rebuilds, validates, re-embeds, and
        ↓                                      copies the result to ../index.html
commit BOTH index.html files together
```

**Never hand-edit anything between these markers** — the build overwrites it:
- `/* === EXERCISES MODULE START … */` … `END === */` → edit `tools/exercises_ui.js` instead
- `/* === READ ALOUD MODULE START … */` … `END === */` → edit `tools/voice_natural.js` + `tools/speech_ui.js` instead

A CI workflow (`.github/workflows/build-check.yml`) fails the push if the committed
`index.html` doesn't match a fresh build — so drift can never sneak in silently again.
...
```
Uganda-spelling/
├── index.html            ← THE DEPLOY FILE (generated — build_all.py copies it here)
├── .github/workflows/    ← CI: rebuilds and fails if the deploy file drifts
└── ple-app/              ← ★ SOURCE OF TRUTH. Edit here.
    ├── index.html        ← the application (data embedded by tools/inject.py)
    ├── notes/            ← one markdown file per study-notes topic
    ├── data/             ← papers / practice / notes / exercises JSON
    ├── curriculum/       ← official NCDC PDFs + extracted text
    ├── docs/             ← design and editing docs
    └── tools/            ← build, validate, inject and patch scripts
```

| Location | Deploys? | Edit here? |
|---|---|---|
| `index.html` (repo root) | **Yes** | No — it is generated by `build_all.py` |
| `ple-app/index.html` | (copied to root) | **Yes** — outside the module markers |
| `ple-app/notes/`, `ple-app/data/` | embedded | **Yes** — then rebuild |
| `ple-app/tools/*.js` | embedded via patchers | **Yes** — then rebuild |

**The old `PLE-3-ready/` deploy folder no longer exists.** The deploy file is
`index.html` at the repo root. If your hosting (Vercel, GitHub Pages…) is still
configured to serve a different folder, update its Root Directory to the repo root.
