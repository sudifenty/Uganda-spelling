...
/* PWA install prompt + #admin route — kept in this file's tail because
   patch_speech re-injects this whole region into index.html on every build */
window.addEventListener('beforeinstallprompt',(e)=>{ e.preventDefault(); state.pwaPrompt=e; if(state.screen==='profile') render(); });
window.addEventListener('appinstalled',()=>{ state.pwaInstalled=true; state.pwaPrompt=null; if(state.screen==='profile') render(); });
async function pwaInstall(){
  if(state.pwaInstalled){ toast('Smart PLE is already installed \u2705'); return; }
  if(state.pwaPrompt){
    try{ state.pwaPrompt.prompt();
      const res=await state.pwaPrompt.userChoice;
      if(res&&res.outcome==='accepted') toast('Installing Smart PLE \u{1F389}');
      state.pwaPrompt=null; render();
    }catch(e){ toast('Install is not available right now.'); }
    return;
  }
  const ua=navigator.userAgent||'';
  if(/iPhone|iPad|iPod/i.test(ua)||(navigator.platform==='MacIntel'&&navigator.maxTouchPoints>1))
    toast('On iPhone/iPad: tap Share \u2192 "Add to Home Screen".');
  else toast('Open your browser menu (\u22EE) \u2192 "Install app" or "Add to Home screen".');
}
if(location.hash==='#admin'){ state.screen='admin'; }
window.addEventListener('hashchange',()=>{ if(location.hash==='#admin'){ state.screen='admin'; render(); } });
spInit();
authInit();
